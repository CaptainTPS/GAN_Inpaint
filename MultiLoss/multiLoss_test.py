...
def main(maskPath = None):
    ...
    newNetGroot = '/home/cad/PycharmProjects/ContextEncoder/MultiLoss/checkpoints/multiloss_inpaintCenter_30_netG.pth'
    newNetG = multiLoss_net.multiLossG(nc, nef, nBottleneck, ngf)
    newNetG.load_state_dict(torch.load(newNetGroot))

    ## set batchNormal to affine false
    newNetG.training = False
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if opt.noiseGen:
            pred = newNetG({Variable(real_cpu), Variable(noise)})
        else:
            pred = newNetG(Variable(real_cpu))
